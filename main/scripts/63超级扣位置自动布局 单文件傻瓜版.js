...
helpervisual={
    ...
    // 大概消耗1 CPU！ 慎用！
    showRoomStructures : function (roomName,structMap,savecpu=true){
        if(savecpu){
            ...
            let visual = new RoomVisual(roomName).clear()
            structMap["road"].forEach(e=>visual.structure(e[0],e[1],STRUCTURE_ROAD));
            _.keys(CONTROLLER_STRUCTURES).forEach(struct=>{
                if(struct=="road"){
                    visual.connectRoads()
                }else{
                    structMap[struct].forEach(e=>visual.structure(e[0],e[1],struct))
                }
            })
        }
